...
-(NSDictionary*) asDictionary {
    ...
    if(_user_data != nil) dict[@"user_data"] = _user_data ;
    if(_user_source != nil) dict[@"user_source"] = _user_source ;
    if(_app_groups != nil){
        if([_app_groups isKindOfClass:[NSArray class]]){
            NSMutableArray * array = [[NSMutableArray alloc] init];
            for( SWGApp *app_groups in (NSArray*)_app_groups) {
    ...
    if(_no_group_apps != nil){
        if([_no_group_apps isKindOfClass:[NSArray class]]){
            NSMutableArray * array = [[NSMutableArray alloc] init];
            for( SWGApp *no_group_apps in (NSArray*)_no_group_apps) {
